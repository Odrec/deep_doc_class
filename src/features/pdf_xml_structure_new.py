...
class Document(object):
    ...
    def process_xml(self):
        ...
        try:
            ...
            num_docs = num_docs + 1
            logger.error("Error %s -- documents with errors %s. Document: %s."\
                         %(err,num_docs,join(dirname(self.xml_path),self.doc_id)))
            e=""
        ...
        try:
            ...
            try:
                ...
                debuglogger.error("XML parsing error in: %s. Number of docs that have failed: %s"%(self.doc_id,num_docs))
                logger.error("Problem parsing xml file %s. Number of docs that have failed: %s"%(self.doc_id,num_docs))
                clean_files(dirname(self.xml_path),self.doc_id)
                return "xml_error"
        clean_files(dirname(self.xml_path),self.doc_id)
    ...
    def get_xml_structure(self):
        ...
        try:
            err = subprocess.Popen(args, stdout=subprocess.PIPE,
                stderr=subprocess.PIPE).communicate(timeout=90)[1].decode(errors="ignore")
