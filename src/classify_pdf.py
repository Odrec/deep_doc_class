...
def checking_for_saved_data(batch_files, batch_ids, batch_labels, batch_quantity, models_path, features_file, metadata, save_preprocessing=True, train=False):
    ...
    if isfile(features_file):
        ...
        __, __, __, __, remaining_ids, remaining_labels, remaining_indexes = da.load_features(models_path, features_file, metadata, batch_ids, batch_labels, save_preprocessing, train)
        remaining_files = []
...
def write_report(final_prediction, batch_ids, count_pages, t_structure, t_deep, t_pred):
    '''
    Generates the final report
    ...
    report_dict['Pages x Documents positively classified'] = positive_classified * sum_pages_positively_classified
    report_dict['Pages x Documents over 0.8'] = documents_prob_08 * sum_pages_over_08
    report_dict['Pages x Documents over 0.6'] = documents_prob_06 * sum_pages_over_06
    report_dict['Pages x Documents over 0.4'] = documents_prob_04 * sum_pages_over_04
    report_dict['Pages x Documents over 0.2'] = documents_prob_02 * sum_pages_over_02
    report_dict['Pages x Documents under 0.2'] = documents_prob_under_02 * sum_pages_under_02
    ...
    report_name_json = 'report_'+timestr+'.json'
    with open(join(paths.RESULTS_PATH, report_name_json), 'w') as fp:
        json.dump(report_dict, fp)
    report_name_csv = 'report_'+timestr+'.csv'
    with open(join(paths.RESULTS_PATH,report_name_csv),'w') as fp:
        w = csv.writer(fp)
        w.writerows(report_dict.items())
    logger.info("Final report saved. %s %s"%(report_name_json, report_name_csv))
        
    
def save_results(batch_ids, predictions):
    '''
    ...
    results_dict = {}
    for i,v in enumerate(final_prediction):
        results_dict[batch_ids[i]] = final_prediction[i]
    if not isdir(paths.RESULTS_PATH): os.makedirs(paths.RESULTS_PATH)
    results_name_json = 'results_'+timestr+'.json'
    with open(join(paths.RESULTS_PATH, results_name_json), 'w') as fp:
        json.dump(results_dict, fp)
    results_name_csv = 'results_'+timestr+'.csv'
    with open(join(paths.RESULTS_PATH,results_name_csv),'w') as fp:
        w = csv.writer(fp)
...
#        save_results = params['save_results']
        results_file_json = params['results_file_json']
        results_file_csv = params['results_file_csv']  
    else: 
    ...
    if num_files > 0:
        ...
        for loop in range(number_of_loops):
            ...
            if batch_files:
                ...
                logger.warning("This could take a long time.")
                features_array[loop], count_pages[loop] = da.get_features(batch_ids, preprocessing_list[loop], preprocessing_list[loop][0], cores, models_path, train)
        ...
        features_array, batch_ids, batch_labels, count_pages, __, __, __ = da.load_features(models_path, features_file, metadata, None, None, False, train)
