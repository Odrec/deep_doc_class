...
def prepare_training_data(class_file='classification.csv', files_path='./files'):
    ...
    directory = dirname(class_file)
    with open(join(directory,"training_data.csv"),"w") as f:
        writer = csv.writer(f)
        writer.writerows(final_data_list)
    print("Created training_data.csv file with %s entries."%(len(final_data_list)))
        
#preapres the metadata. Creates a new csv with the document id,
# filename and folder names for all documents on the metadata file
#
#@param meta_file:       the metadata.csv file
#
#@result:                creates a metadata_clean.csv file with the required metadata 
def prepare_metadata(meta_file='metadata.csv'):
    
    if not isfile(meta_file): hlp()
    file_directory = dirname(meta_file)
    
    with open(meta_file) as cf:
        data = csv.DictReader(cf)
        new_data = []
        for i,row in enumerate(data):
            if i==24:print(row['document_id'], row['filename'], row['folder_name'])
            new_data.append([row['document_id'], row['filename'], row['folder_name']])
            
    with open(join(file_directory,"metadata_clean.csv"),"w") as f:
        writer = csv.writer(f)
        writer.writerow(['document_id', 'filename', 'folder_name'])
        writer.writerows(new_data)
...
def hlp():
    ...
    print("\n")
    print("-meta [path_to_metadata_file]")
    print("\n")
    print("This will prepare the metadata file with only the required columns. Make sure the file has as headers at least the fields \
          of 'document_id', 'filename' and 'folder_name'")
    print("\n")
...
if '-h' in args:
    ...
    elif '-meta' in args and len_args == 3: 
        prepare_metadata(args[2])  
    else: hlp()
else: hlp()