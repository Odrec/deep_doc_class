...
import features.pdf_xml_structure as fxs
...
def get_metadata(batch_files, metadata, extradata):
    '''
    ...
    @param extradata: data related to courses and number of participants
    @dtype extradata: dict
    ...
    for i, f in enumerate(batch_files):
        ...
        if doc_id in metadata.keys():
            ...
            if extradata: batch_extra[doc_id] = extradata[doc_id]
        else:
            logger.warning("No metadata found for file with id: %s",doc_id)
            batch_meta[doc_id] = {}
            batch_meta[doc_id]['file_name'] = " " 
            batch_meta[doc_id]['folder_name'] = " "
            batch_extra[doc_id] = {}
            batch_extra[doc_id]['course_name'] = " " 
            batch_extra[doc_id]['number_participants'] = 0
    return batch_meta, batch_extra  

def get_preprocessing_data(batch_files, metadata, extra, pool_structure, pool_properties):
    '''
    ...
    @param pool_structure: multiprocessing pool for structure features
    @dtype pool_structure: Pool
    
    @param pool_properties: multiprocessing pool for properties features
    @dtype pool_properties: Pool
...
def get_features(doc_ids, preprocessing_list, metadata, models_path, extra={}, train=False, overwrite=False):
    ...
    for i,f in enumerate(features):
        debuglogger.debug("Shape of feature number %s: %s"%(str(i),str(features[i].shape)))
    return features, count_pages, courses, number_participants
    
def save_features(numeric_features, preprocessing_list, bow_features_names, numeric_features_names, batch_files, batch_ids, batch_labels,\
                  features_file, number_participants=None, course_name=None, count_pages=[], overwrite=False):
    '''
    Save the features
    ...
    for i,did in enumerate(batch_ids):
        ...
        if number_participants: features_dict[did]['number_participants'] = number_participants[i]
        if course_name: features_dict[did]['course_name'] = course_name[i]
...
def load_features(models_path, features_file, metadata, batch_ids=None, batch_labels=None, save=False, train=False):
    ...
    @return number_participants: number of participants associated to each file
    @rtype number_participants: list
    
    @return course_name: list of name of courses associated to each file
    @rtype course_name: list
    ...
    @return remaining_indexes: list of indexes of documents that still need to be processed
    @rtype remaining_indexes: list
    '''
    ...
    if isfile(features_file):        
        ...
        for i,did in enumerate(doc_ids):
            bow_data[did] = {}
            if 'number_participants' in json_data[did]: number_participants.append(json_data[did]['number_participants'])
            if 'course_name' in json_data[did]: course_name.append(json_data[did]['course_name'])
            for bf in bow_features_names:
    ...
    return feature_data, doc_ids, doc_labels, count_pages, number_participants, course_name, remaining_ids, remaining_labels, remaining_indexes
...
def split_data_for_training(number_of_files, features, ids, labels):
    ...
    logger.info("Using 80% of documents ("+str(quant_train_data)+") or training and 20% of documents ("+str(quant_test_data)+") for testing.")
...
def get_images_from_files(data_path, batch_ids, train=False):
    ...
    if not isfile(file1) or not isfile(file2) or not isfile(file3):
        debuglogger.error("One or more or these files is missing! %s %s %s"%(file1, file2, file3))
        logger.error("One or more or these files is missing! %s %s %s"%(file1, file2, file3))
        sys.exit(1)
